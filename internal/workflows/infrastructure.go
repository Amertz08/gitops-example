...
func SpinUpWorkflow(ctx workflow.Context, input SpinUpInput) error {
	...
	if err := workflow.ExecuteActivity(ctx, aws.CreateEKSCluster, input.Region, input.ClusterName, vpcID, subnetIDs).
		Get(ctx, nil); err != nil {
		return err
	}

	if err := workflow.ExecuteActivity(ctx, aws.CreateNodeGroup, input.Region, input.ClusterName, subnetIDs, input.NodeCount, input.NodeInstanceType).
		Get(ctx, nil); err != nil {
		return err
	}
...
func SpinDownWorkflow(ctx workflow.Context, input SpinDownInput) error {
	...
	if err := workflow.ExecuteActivity(ctx, aws.DeleteSubnets, input.Region, vpcID).
		Get(ctx, nil); err != nil {
		return err
	}

	if err := workflow.ExecuteActivity(ctx, aws.DetachDeleteInternetGateway, input.Region, vpcID).
		Get(ctx, nil); err != nil {
		return err
	}

	if err := workflow.ExecuteActivity(ctx, aws.DeleteVPC, input.Region, vpcID).
		Get(ctx, nil); err != nil {
		return err
	}
